...
void CarlaClient::ImgCbk(carla::SharedPtr<carla::sensor::SensorData> rawSensorDataPtr) {
	...
	uint8_t *image_buffer = new uint8_t[img_size];
	int index{0};
	...
	cv::Mat temp_img(ImagePtr->GetHeight(),ImagePtr->GetWidth(),CV_8SC3,image_buffer);
	cv::imshow("tmp_image",temp_img);
	cv::waitKey(500);
}
...
int main(int argc, char** argv){
	...
	CarlaClient cc("localhost", 2000,argc,argv);
